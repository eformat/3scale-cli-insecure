...
exports.listActiveDocs = function(){
  ...
  return response.then(function (r) {
    ...
    if (res.statusCode >= 300) {
      ...
      cli.debug({name: '3scale-cli:activedocs:list', msg: 'ERROR:' +r[0].statusCode+" %o", data: (body.errors || body.status)})
...
exports.createActiveDocs = function(file, system_name){
  ...
  .then(function(api){
    ...
    return response.then(function (r) {
      ...
      if (res.statusCode >= 300) {
        cli.error({message:"ERROR encountered: " + JSON.stringify(body.error || body.status)});
        cli.debug({name: '3scale-cli:activedocs:create', msg: 'ERROR:' +r[0].statusCode+" %o", data: (body.errors || body.status)})
        throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
      } else {
...
exports.updateActiveDocs = function(file, id){
  ...
  .then(function(api){
    ...
    return response.then(function (r) {
      ...
      if (res.statusCode >= 300) {
        cli.error({message:"ERROR encountered: " + JSON.stringify(body.error || body.status)});
        cli.debug({name: '3scale-cli:activedocs:update', msg: 'ERROR:' +r[0].statusCode+" %o", data: (body.errors || body.status)})
        throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
      } else {
